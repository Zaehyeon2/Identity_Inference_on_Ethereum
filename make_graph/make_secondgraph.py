...
normal_first_graphs = glob.glob("Normal first-order nodes/*.csv")[1:]
...
for f in normal_first_graphs:
    ...
    with open("Normal first-order nodes/"+file_name, "r") as ff:
        while True:
            ...
            normal_graph = glob.glob("Normal second-order nodes/"+file_name[:-4]+"/*.csv")
            for ff2 in (normal_graph):
                file_2name = ff2.split("\\")[1]
                with open("Normal second-order nodes/"+file_name[:-4]+"/"+file_2name, "r") as fff:
                    while True:
                        arg = fff.readline()
                        if not arg: break
                        args = arg.split(",")
    ...
    with open("Phishing first-order nodes/"+file_name, "r") as ff:
        ...
        while True:
            ...
            if not (args[4] in node_list.keys()):
                ...
                node_idx += 1
            graph.add_edges_from([(node_list[args[3]], node_list[args[4]], dict(weight=float(args[5]), timestamp=int(args[2])))])
            phishing_graph = glob.glob("Phishing second-order nodes/"+file_name[:-4]+"/*.csv")
            for ff2 in phishing_graph:
                file_2name = ff2.split("\\")[1]
                with open("Phishing second-order nodes/"+file_name[:-4]+"/"+file_2name, "r") as fff:
                    while True:
                        arg = fff.readline()
                        if not arg: break
                        args = arg.split(",")
...
with open("second_graph.pikle", "wb") as f:
